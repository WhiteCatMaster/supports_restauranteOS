...
void cerrar_cocina(int senial) {
  ...
  resultado = sem_destroy(&semCocina);
  if (resultado == -1) {
...
void *prepararIngredientes(void *args) {
  ...
  while (1) {
    ...
    // Una cola es como un buzón para pasar mensajes entre procesos, la sala
    // mete un mensaje en el buzón y la cocina los recibe mediante "mq_receive"
    // y "mq_send"

    // "mq_receive" va a leer la cola "mq_cocina_fd" y va a guardar en "buffer"
    // (variable creada con antelación) "MSG_BUFFER_SIZE" va a indicar el tamaño
    // máximo de bytes que "mq_receive" puede copiar en buffer "NULL" indica que
    // no hay prioridad de mensaje
    bytes_read = mq_receive(mq_cocina_fd, buffer, MSG_BUFFER_SIZE, NULL);

    // Si el "bytes_read" da -1 significa que ha habido un error al intentar
    // leer la cola
    if (bytes_read == -1) {
    ...
    if (bytes_read >= MSG_BUFFER_SIZE) {
      // Cortarlo donde esté el tamaño máximo y le ponerle el final ("\0") ahi
      // mismo
    ...
    // Indicar a la función "cocinar" que los ingredientes están listos,
    // poniendo el semáforo a 1
...
void *cocinar(void *arg) {
  ...
  while (1) {
    // El semáforo hace que el programa no siga hasta que no estén los
    // ingredientes listos
    ...
    // Misma lógica que el semáforo anterior, solo que esta vez se ocupa de que
    // el emplatado no se haga hasta que los ingredientes hayan sido cocinados
    sem_post(&semCocina);
  }
}
  ...
  if (pid_sala != 0) {
    ...
    if (pid_cocina != 0) {
      ...
      do {
        ...
        // muera, luego el PID del hijo se guardará en "pidFin"
        pidFin = wait(&estado);
        ...
        // El padre seguira dando vueltas al "do-while" hasta que ambos hijos
        // hayan muerto
      ...
      // El primer 0 indica que el semáforo será privado de los hilos de un
      // proceso El segundo 0 es el valor inicial del semáforo, indicando que la
      // "barrera" estará cerrada al empezar
      ...
      // "0644" son permisos de seguridad y "&attr" son las reglas de
      // configuración definidas en las lineas anteriores
      mq_cocina_fd = mq_open(COLACOCINA, O_CREAT | O_RDONLY, 0644, &attr);

      printf("[Cocina] Comienzo de la preparación de platos...\n");

      // Inicializar los hilos definidos anteriormente, especificandoles que
      // función de la cocina hará cada uno
      ...
      // Hacen que el proceso cocina espere a que los tres hilos hayan terminado
      // (a que todos los trabajos de la cocina hayan sido terminados) antes de
      // poder cerrarse
    ...
    // Se crea la cola de mensajes, en este caso con "O_WRONLY", indicando que
    ...
    // Bucle infinito que cada cierto tiempo simula la llegada de una comanda y
    // la envía a la cocina mediante una cola de mensajes
    while (1) {
